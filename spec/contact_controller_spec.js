...
describe( "ContactController", () => {
  ...
  describe( "#delete()", () => {
    ...
    /* test values */
    const deckard = ["Rick Deckard", "000-000-000", "null@null.com"];

    it( "should not remove any contacts that do not match the ID passed",
      ( done ) => {
        this.book.addContact( ...deckard ).then( () => {
          this.book.getContacts()
          .then( ( contacts ) => {
            expect( contacts[ 0 ].name ).toBe( deckard[ 0 ] );
            expect( contacts.length ).toBe( 1 );
    ...
    it( "should remove the contact that matches the ID passed",
      ( done ) => {
        this.book.addContact( ...deckard ).then( () => {
          this.book.getContacts()
          .then( ( contacts ) => {
            expect( contacts[ 0 ].name ).toBe( deckard[ 0 ] );
            expect( contacts.length ).toBe( 1 );
            this.book.delete( contacts[ 0 ].id ).then( () => {
              this.book.getContacts()
              .then( ( contacts ) => {
                expect( contacts.length ).toBe( 0 );
                done();
